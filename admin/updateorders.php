...
$sql="select orders_id,orders_state,orders_data,pay_state,users.users_name,goods.goods_name from orders,users,goods where users.users_id=orders.users_id and goods.goods_id=orders.goods_id and orders_id=".$_GET['ordersid'];
$rs = $conn->query($sql);
$att1= $rs->fetch_row();
?>
<!DOCTYPE html>
...
<section class="wrapper">
  ...
  <div class="panel panel-default">
    ...
    <div class="table-responsive">
     ...
     <form action="ctrl/updateorders.php" role="form" method="post">
    <input type="hidden" class="form-control" name="ordersid" value="<?php echo $att1[0];?>"/>
    ...
    $attstate=array("1"=>"已发货","2"=>"未发货");
     ...
     <div class="form-group has-warning has-feedback">
      ...
      <input type="date" class="form-control" name="ordersdata" value="<?php echo $att1[3];?>"/>
    ...
    $paystate=array("1"=>"已付款","2"=>"未付款");
     ...
     foreach ($attgoods as $v){
         //小类表的大类名称与大类表的大类名称进行比较
         if($att1[6]==$v[3]){
             echo "<option value='".$v[0]."' selected='selected'>".$v[3]."</option>";
         }else{
